...
const statusMap: Record<string, { label: string; color: string }> = {
  open: { label: "Aberto", color: "bg-blue-950/60 text-blue-300" },
  in_progress: { label: "Em Progresso", color: "bg-amber-950/60 text-amber-300" },
  waiting_customer: { label: "Aguard. Cliente", color: "bg-orange-950/60 text-orange-300" },
  waiting_approval: { label: "Aguard. Aprovação", color: "bg-purple-950/60 text-purple-300" },
  resolved: { label: "Resolvido", color: "bg-emerald-950/60 text-emerald-300" },
  closed: { label: "Fechado", color: "bg-slate-800 text-slate-400" },
  escalated: { label: "Escalado", color: "bg-red-950/60 text-red-300" },
};

const phaseMap: Record<string, { label: string; color: string }> = {
  greeting: { label: "Saudação", color: "bg-sky-950/60 text-sky-300" },
  understanding: { label: "Compreensão", color: "bg-blue-950/60 text-blue-300" },
  collecting_evidence: { label: "Coletando Evidência", color: "bg-amber-950/60 text-amber-300" },
  validating: { label: "Validando", color: "bg-indigo-950/60 text-indigo-300" },
  diagnosing: { label: "Diagnosticando", color: "bg-purple-950/60 text-purple-300" },
  proposing_solution: { label: "Propondo Solução", color: "bg-teal-950/60 text-teal-300" },
  awaiting_confirmation: { label: "Aguard. Confirmação", color: "bg-orange-950/60 text-orange-300" },
  closing: { label: "Fechamento", color: "bg-emerald-950/60 text-emerald-300" },
  escalated_human: { label: "Escalado (Humano)", color: "bg-red-950/60 text-red-300" },
  escalated_dev: { label: "Escalado (Dev)", color: "bg-red-950/60 text-red-300" },
};
      ...
      <div className={`max-w-[80%] space-y-1`}>
        <div
          className={`rounded-2xl px-4 py-2.5 text-sm leading-relaxed ${
            isAgent
              ? "rounded-tl-sm bg-slate-900/80 text-slate-200 shadow-sm border border-slate-800"
...
export default function TicketDetailPage() {
  ...
  if (error || !ticket) {
    return (
      <div className="space-y-4">
        <button onClick={() => router.back()} className="inline-flex items-center gap-1.5 text-sm text-slate-500 hover:text-slate-100">
  ...
  const phase = phaseMap[ticket.conversationPhase || "greeting"] || { label: ticket.conversationPhase, color: "bg-slate-800 text-slate-400" };
  const status = statusMap[ticket.status] || { label: ticket.status, color: "bg-slate-800 text-slate-400" };
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Header bar */}
      <div className="flex flex-wrap items-center justify-between gap-3 rounded-lg bg-slate-900/80 px-5 py-3 shadow-sm border border-slate-800">
        <div className="flex items-center gap-3">
          <h1 className="text-lg font-semibold text-slate-100 truncate max-w-md">{ticket.title}</h1>
        ...
        <div className="flex items-center gap-2 text-xs text-slate-500">
          {ticket.systemName && <span className="font-medium text-slate-300">{ticket.systemName}</span>}
      ...
      <div className="grid grid-cols-1 gap-4 xl:grid-cols-3">
        ...
        <div className="flex flex-col xl:col-span-2" style={{ height: "calc(100vh - 220px)" }}>
          <Card className="flex flex-1 flex-col overflow-hidden">
            <CardHeader className="shrink-0 border-b border-slate-800 py-3">
            ...
            {/* Test input */}
            <div className="shrink-0 border-t border-slate-800 bg-slate-900/80 p-3">
              <div className="flex gap-2">
                <input
                  ...
                  className="flex-1 rounded-lg border border-slate-800 bg-slate-950/50 px-3 py-2 text-sm text-slate-200 outline-none focus:border-blue-400 focus:ring-1 focus:ring-blue-400 disabled:bg-slate-900/50 disabled:text-slate-500"
                />
                <Button
            ...
            <CardContent className="space-y-3 text-sm">
              <div className="grid grid-cols-2 gap-3">
                <div className="rounded-lg bg-slate-900/50 p-3 text-center">
                  <p className="text-2xl font-bold text-slate-200">{ticket.attemptCount || 0}</p>
                  <p className="text-[10px] font-medium text-slate-400 uppercase tracking-wider">/ 3 tentativas</p>
                </div>
                <div className="rounded-lg bg-slate-900/50 p-3 text-center">
                  <p className={`text-xs font-semibold ${
                    ticket.evidenceStatus === "sufficient" ? "text-emerald-400" :
                    ticket.evidenceStatus === "insufficient" ? "text-red-400" :
                    ticket.evidenceStatus === "requested" ? "text-amber-400" :
                    "text-slate-500"
                  }`}>
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    {ticket.attempts.map((a) => (
                      <div key={a.attemptNumber} className="rounded-lg border border-slate-800 bg-slate-900/80 p-3">
                        <div className="flex items-center justify-between">
                          ...
                          <span className={`rounded-full px-2 py-0.5 text-[10px] font-medium ${
                            a.outcome === "success" ? "bg-emerald-950/60 text-emerald-300" :
                            a.outcome === "failed" ? "bg-red-950/60 text-red-300" :
                            "bg-amber-950/60 text-amber-300"
                          }`}>
                            {a.outcome === "success" ? "Sucesso" : a.outcome === "failed" ? "Falhou" : "Pendente"}
                          </span>
                        </div>
                        <p className="mt-1.5 text-xs text-slate-400 line-clamp-2">{a.solution}</p>
                        {a.clientFeedback && (
                        ...
                        {a.knowledgeSourcesUsed.length > 0 && (
                          <div className="mt-1.5 flex flex-wrap gap-1">
                            {a.knowledgeSourcesUsed.map((s, i) => (
                              <span key={i} className="rounded bg-slate-800 px-1.5 py-0.5 text-[10px] text-slate-400">{s}</span>
                            ))}
                          </div>
                        )}
                      </div>
              ...
              <CardContent className="space-y-2">
                {ticket.escalations.map((e, i) => (
                  <div key={i} className="rounded-lg border border-red-900/50 bg-red-950/30 p-3">
                    ...
                    <p className="mt-1 text-xs text-red-400">{e.reason}</p>
                    {e.clickupUrl && (
                      <a href={e.clickupUrl} target="_blank" rel="noopener noreferrer" className="mt-1 inline-flex items-center gap-1 text-[10px] text-blue-400 hover:underline">
              ...
              <CardHeader className="py-3 cursor-pointer" onClick={() => setShowKnowledgeHits(!showKnowledgeHits)}>
                <CardTitle className="flex items-center justify-between text-sm">
                  <span className="flex items-center gap-2">
                    ...
                    <span className="rounded-full bg-teal-950/60 px-2 py-0.5 text-[10px] font-medium text-teal-300">
                  ...
                  {ticket.knowledgeHits.map((h, i) => (
                    <div key={i} className="rounded-lg bg-slate-900/50 p-2.5">
                      <div className="flex items-start justify-between gap-2">
                        <p className="text-xs font-medium text-slate-300 line-clamp-1">{h.title}</p>
                        {h.relevanceScore != null && (
                          <span className="shrink-0 rounded bg-teal-950/60 px-1.5 py-0.5 text-[10px] text-teal-300">
                            {Math.round(h.relevanceScore * 100)}%
                          </span>
                        )}
                      </div>
                      <div className="mt-1 flex items-center gap-2 text-[10px] text-slate-400">
                        <span className="rounded bg-slate-800 px-1 py-0.5 text-slate-300">{h.source}</span>
                ...
                <CardContent>
                  <div className="space-y-1.5 max-h-64 overflow-y-auto">
                    {ticket.decisionTrace.map((d, i) => (
                      <p key={i} className="rounded bg-slate-900/50 px-2.5 py-1.5 text-[11px] text-slate-400 font-mono leading-snug">
                        {d}
                      </p>
                    ))}
                  </div>
              ...
              <CardContent className="space-y-2 text-sm">
                ...
                {ticket.customer.email && (
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-slate-400 text-xs">{ticket.customer.email}</span>
                  </div>
                )}
                {ticket.customer.company && (
                  <div className="flex items-center gap-2">
                    <Building2 className="h-3.5 w-3.5 text-slate-400" />
                    <span className="text-slate-300">{ticket.customer.company}</span>
                  </div>
                )}
              </CardContent>
            </Card>
