...
export default function UsersPage() {
  ...
  const roleBadge = (role: string) => {
    const colors: Record<string, string> = {
      ...
      analyst: "bg-blue-950/60 text-blue-300",
        ...
        <div className="rounded-xl border border-blue-200 bg-blue-50/50 p-5">
          <div className="mb-4 flex items-center justify-between">
            ...
            <button
              ...
              className="rounded-lg p-1.5 text-slate-400 hover:bg-slate-900/80 hover:text-slate-400"
            >
              <X className="h-4 w-4" />
            </button>
          </div>
          ...
          {createError && (
            <div className="mb-3 rounded-lg border border-red-800/60 bg-red-950/40 px-4 py-2.5 text-sm text-red-300">
              {createError}
            </div>
          )}
          {createSuccess && (
            <div className="mb-3 rounded-lg border border-emerald-800/60 bg-emerald-950/40 px-4 py-2.5 text-sm text-emerald-300">
              {createSuccess}
            </div>
          )}
          ...
          <form onSubmit={handleCreate} className="grid grid-cols-1 gap-3 sm:grid-cols-2">
            <div>
              ...
              <input
                ...
                required
                className="w-full rounded-lg border border-slate-700 bg-slate-900/80 px-3 py-2 text-sm outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500"
        ...
        <div className="rounded-xl border border-amber-800/60 bg-amber-950/30 p-5">
          ...
          <div className="space-y-3">
            {pending.map((u) => (
              <div
                key={u.id}
                className="flex items-center justify-between rounded-lg border border-amber-200 bg-slate-900/80 p-4"
              >
                <div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  {u.role !== "admin" && (
                    <button
                      ...
                      disabled={actionLoading === u.id}
                      className="rounded-lg p-1.5 text-slate-400 transition-colors hover:bg-slate-800/50 hover:text-slate-400 disabled:opacity-60"
